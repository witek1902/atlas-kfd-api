...
$$
DECLARE
  section_id bigint := (SELECT id FROM public.exercise_section WHERE title = 'Nogi i pośladki');
  exercise_codes bigint[] := ARRAY(SELECT id FROM public.exercise WHERE code LIKE 'NOGI%');
BEGIN
