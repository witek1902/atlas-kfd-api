...
$$
DECLARE
  section_id bigint := (SELECT id FROM public.exercise_section WHERE title = 'Barki');
  exercise_codes bigint[] := ARRAY(SELECT id FROM public.exercise WHERE code LIKE 'B%');
BEGIN
