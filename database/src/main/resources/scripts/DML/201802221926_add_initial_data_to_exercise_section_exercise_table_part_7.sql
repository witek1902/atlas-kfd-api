...
$$
DECLARE
  section_id bigint := (SELECT id FROM public.exercise_section WHERE title = 'Przedramiona');
  exercise_codes bigint[] := ARRAY(SELECT id FROM public.exercise WHERE code LIKE 'PR%');
BEGIN
