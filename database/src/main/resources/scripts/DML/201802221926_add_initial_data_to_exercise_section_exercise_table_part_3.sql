...
$$
DECLARE
  section_id bigint := (SELECT id FROM public.exercise_section WHERE title = 'Brzuch');
  exercise_codes bigint[] := ARRAY(SELECT id FROM public.exercise WHERE code LIKE 'BRZUCH%');
BEGIN
