...
$$
DECLARE
  section_id bigint := (SELECT id FROM public.exercise_section WHERE title = 'Klatka piersiowa');
  exercise_codes bigint[] := ARRAY(SELECT id FROM public.exercise WHERE code LIKE 'KLATA%');
BEGIN
